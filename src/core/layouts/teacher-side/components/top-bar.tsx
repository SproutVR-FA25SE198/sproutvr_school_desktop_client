...
import { User, KeyRound, LogOut, ChevronDown, FileBadge, Presentation } from 'lucide-react';
...
const navItems: NavItem[] = [
  { label: 'Bài giảng', href: routes.home, icon: <FileBadge /> },
  { label: 'Phiên học VR', href: routes.sessionList, icon: <Presentation /> },
];
      ...
      <div className='bg-white border-b border-neutral-200 px-8 py-4 flex items-center justify-between shadow-sm relative z-50'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div className='w-16 h-16 flex items-center justify-center p-2 shrink-0 '>
            <img src={logo} alt='SproutVR Logo' className='w-full h-full object-contain' />
          </div>
          ...
          {/* Navigation */}
          <nav className='flex items-center gap-1 ml-8 border-l border-neutral-200 pl-8'>
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && <span className='absolute bottom-0 left-0 right-0 h-1 bg-primary rounded-full' />}
                </Link>
              );
            })}
          </nav>
