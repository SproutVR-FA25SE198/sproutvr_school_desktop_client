        ...
        <div className='max-w-4xl mx-auto px-8 pt-8 pb-24 space-y-8'>
              ...
              <div>
                ...
                <div className='flex items-center gap-3 text-neutral-600'>
                  ...
                  <span
                    ...
                    style={{
                      color: accountDetail.status.key === AccountStatus.Active ? '#10b981' : '#f59e0b',
                    }}
                  >
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                <div>
                  ...
                  <div className='bg-neutral-50 rounded-lg px-4 py-3 border border-neutral-200'>
                    <p className='text-sm text-neutral-900 font-mono break-all'>{accountDetail.fullName}</p>
                  </div>
                </div>
                <div>
                  <label className='text-xs font-semibold text-neutral-500 uppercase tracking-wider'>Email</label>
                  <div className='bg-neutral-50 rounded-lg px-4 py-3 border border-neutral-200'>
                    <p className='text-sm text-neutral-900 font-mono break-all'>{accountDetail.email}</p>
                  </div>
                </div>
              </div>
              <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          ...
          <Card className='overflow-hidden'>
            <div className='flex items-center justify-between bg-neutral-50 px-8 py-5 border-b border-neutral-200'>
              ...
              <p className='text-sm text-neutral-600 font-medium'>Tổng số: {accountDetail.lessons.length}</p>
            </div>
            <div className='p-8 space-y-4'>
              {accountDetail.lessons.length === 0 ? (
                <p className='text-neutral-500 text-sm text-center'>Không có bài học nào.</p>
              ) : (
                    ...
                    <div className='flex items-center gap-3'>
                      <div
                        className={`w-2 h-2 rounded-full ${
                          lesson.status === AccountDetailLessonStatus.Active ? 'bg-green-500' : 'bg-neutral-400'
                        }`}
                      />
                      <p className='text-sm font-semibold text-neutral-900'>{lesson.name}</p>
                    </div>
                    <Badge variant={lesson.status === AccountDetailLessonStatus.Active ? 'secondary' : 'default'}>
                      {lesson.status === AccountDetailLessonStatus.Active ? 'Hoạt động' : 'Không hoạt động'}
                    </Badge>
                  </div>
                ))
              )}
          ...
          <Card className='overflow-hidden'>
            <div className='flex items-center justify-between bg-neutral-50 px-8 py-5 border-b border-neutral-200'>
              ...
              <p className='text-sm text-neutral-600 font-medium'>Tổng số: {accountDetail.vrLearningSessions.length}</p>
            </div>
            <div className='p-8 space-y-4'>
              {accountDetail.vrLearningSessions.length === 0 ? (
                <p className='text-neutral-500 text-sm text-center'>Không có buổi học nào.</p>
              ) : (
                accountDetail.vrLearningSessions.map((session) => (
                  <div
                    ...
                    className='flex items-center justify-between p-4 rounded-lg bg-neutral-50 border border-neutral-200'
                  >
                    <p className='text-sm font-semibold text-neutral-900'>{session.className}</p>
                    <p className='text-xs text-neutral-600'>{formatDateTime(session.createdAtUtc)}</p>
                  </div>
                ))
              )}
            </div>
